"""
    ...
    ** HumanEngineMatch is a custom addition to the standard pystockfish.py file, 
    not done by the person above. **
"""

import subprocess
from random import randint

class HumanEngineMatch:
    '''
        This is a custom class, which draws upon the format of the match class below, 
        but is compatible for a match between a human (or custom moves) and a chess engine.
        
        HumanEngineMatch sets up a match between a human (white) and an engine (black).
        -The first input must be the human. 
        -The engine used has only been tested with the engine class in this file.
            -Must initialize class with engine.
            -the only thing that matters is depth= int, default = 2, higher = smarter
            
        -self.move advances the game by two moves: the entered user move, and the engine's response
        
        -self.move returns engines's response to player's move
        
        -To start match in the middle, you set self.moves equal to a list of the sequential moves
        
        -moves are : "a4a6" meaning piece from a4 to a6, standard chessboard setup (white in 1&2)
    ...
    def move(self, white_move):
        ...
        if ponder != '(none)': return bestmove
        else:
